"""
Gothenburg CTM — mode='links' only.
v3: all interior junctions inject+absorb (threshold=1.0),
    prod_scale=1e-4 (10× injection), attr_scale=1e-5 (absorption unchanged).

Simulates 120 minutes (7200 s) with 60 snapshots × 2 min each.

Outputs (all in ./output/):
  gothenburg_avg_speed_v3.png
  gothenburg_queue_length_v3.png
  gothenburg_load_factor_v3.png
  gothenburg_total_delay_v3.png
  gothenburg_links_v3.gif
"""
import sys
import os
import io
...
# ── 2. Build network (mode=links, all junctions, 10× production) ─────────────
print("\nBuilding mode=links network …")
...
print(f"\nSimulating {T_SIM/60:.0f} min — {N_FRAMES} snapshots × {DT_SNAP:.0f} s ({DT_SNAP:.0f} s each) …")
...
def avg_speed(net, m):
    """Length-weighted mean speed across all links (m/s)."""
...
def queue_length(net, m):
    """Total vehicles on congested links (rho > rho_critical) (veh)."""
    q = 0.0
    for eid, link in net.links.items():
...
# ── 6. Metric plots ───────────────────────────────────────────────────────────
METRICS = [
    ('Average Speed',       speeds,  'm/s',   'gothenburg_avg_speed_v3.png'),
    ('Queue Length',        queues,  'veh',   'gothenburg_queue_length_v3.png'),
    ('Load Factor (ρ/K)',   loads,   '—',     'gothenburg_load_factor_v3.png'),
    ('Total Network Delay', delays,  'veh·s', 'gothenburg_total_delay_v3.png'),
]

for title, vals, ylabel, fname in METRICS:
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.plot(times, vals, linewidth=2, color='steelblue')
    ax.set_xlabel('Time (s)')
    ax.set_ylabel(f'{title} ({ylabel})')
    ax.set_title(f'Gothenburg CTM v3 — {title}\n'
                 f'mode=links | all junctions | prod_scale=1e-4 | attr_scale=1e-5')
...
print("Rendering gothenburg_links_v3.gif …")
frames = []
for t, m in snapshots:
    ...
    ax.set_title(f'mode=links  (all junctions, prod×10)   t = {t:.0f}s')
    ax.set_xlabel('X (m)'); ax.set_ylabel('Y (m)')
    fig.tight_layout()
    frames.append(_frame_to_pil(fig))

gif_path = os.path.join(OUT_DIR, 'gothenburg_links_v3.gif')
frames[0].save(gif_path, save_all=True, append_images=frames[1:],
               duration=250, loop=0)
print("Saved → output/gothenburg_links_v3.gif")
...
print(f"\n{'='*68}")
print(f"  FINAL STATS at t = {T_SIM/60:.0f} min")
print(f"  mode=links | all junctions | prod_scale=1e-4 | attr_scale=1e-5")
print(f"{'='*68}")
