"""
Gothenburg CTM — mode='links', hora de ponta dinâmica.
v4: prod_scale varia no tempo (hora de ponta), attr_scale=1e-5 constante.
    Todos os junctions interiores injetam e absorvem (threshold=1.0).

Perfil de produção:
  0  – 20 min  →  10×  (prod_scale = 1e-4)
  20 – 60 min  →  50×  (prod_scale = 5e-4)   ← pico manhã
  60 – 80 min  → 100×  (prod_scale = 1e-3)   ← pico máximo
  80 – 120 min →  10×  (prod_scale = 1e-4)   ← retorno

Linhas verticais nos gráficos marcam as transições.

Outputs (./output/):
  gothenburg_avg_speed_v4.png
  gothenburg_queue_length_v4.png
  gothenburg_load_factor_v4.png
  gothenburg_total_delay_v4.png
  gothenburg_links_v4.gif
"""
import sys
import os
import io
...
from ctm.junction import SourceNode, InjectionJunction, InjectionAbsorptionJunction

OUT_DIR = os.path.join(REPO_ROOT, 'output')
os.makedirs(OUT_DIR, exist_ok=True)

# ── hora de ponta: transições em segundos ─────────────────────────────────────
PEAK_TRANSITIONS = [
    (0,    20 * 60, 1e-4),   # 0–20 min   → 10×
    (20 * 60, 60 * 60, 5e-4),  # 20–60 min  → 50×
    (60 * 60, 80 * 60, 1e-3),  # 60–80 min  → 100×
    (80 * 60, 120 * 60, 1e-4), # 80–120 min → 10×
]

def peak_factor(t):
    """Return prod_scale for simulation time t (s)."""
    for t_start, t_end, scale in PEAK_TRANSITIONS:
        if t_start <= t < t_end:
            return scale
...
# ── 2. Build network com prod_scale base (será substituído dinamicamente) ─────
print("\nBuilding network …")
net = build_network_od(
    road_data, V, cen, z_cars, z_employ,
    mode='links',
    T_max=180,
    prod_scale=1e-4,   # base — será overridden dinamicamente
    attr_scale=1e-5,
...
# ── 3. Guardar taxas base de cada nó injetor (production × 1.0) ──────────────
# demand_fn(t) = base_rate × (peak_factor(t) / 1e-4)
# Extraímos base_rate = demand_fn(0) para cada SourceNode e InjectionNode
source_base  = {}   # nid → base rate (veh/s) at prod_scale=1e-4
inject_base  = {}   # nid → {fid: base extra_inflow}

for nid, nd in net.nodes.items():
    if isinstance(nd, SourceNode):
        source_base[nid] = nd.demand_fn(0.0)
    elif isinstance(nd, (InjectionJunction, InjectionAbsorptionJunction)):
        inject_base[nid] = dict(nd.extra_inflow)

def apply_peak(t):
    """Update all injection rates to match peak_factor(t)."""
    scale = peak_factor(t)
    ratio = scale / 1e-4   # relative to base prod_scale

    for nid, base_rate in source_base.items():
        nd = net.nodes[nid]
        new_rate = base_rate * ratio
        nd.demand_fn = lambda tt, r=new_rate: r

    for nid, base_inflow in inject_base.items():
        nd = net.nodes[nid]
        nd.extra_inflow = {fid: v * ratio for fid, v in base_inflow.items()}
...
# transições em segundos para linhas verticais
TRANSITIONS = [t * 60 for t in [20, 60, 80]]
LABELS      = ['×10→×50', '×50→×100', '×100→×10']

# ── 7. Metric plots com linhas de transição ───────────────────────────────────
METRICS = [
    ('Average Speed',       speeds,  'm/s',   'gothenburg_avg_speed_v4.png'),
    ('Queue Length',        queues,  'veh',   'gothenburg_queue_length_v4.png'),
    ('Load Factor (ρ/K)',   loads,   '—',     'gothenburg_load_factor_v4.png'),
    ('Total Network Delay', delays,  'veh·s', 'gothenburg_total_delay_v4.png'),
]
...
for title, vals, ylabel, fname in METRICS:
    ...
    for t_trans, lbl, col in zip(TRANSITIONS, LABELS, colors):
        ax.axvline(t_trans, color=col, linestyle='--', linewidth=1.4, zorder=2)
        ax.text(t_trans + 30, ax.get_ylim()[1] if ax.get_ylim()[1] != 0 else 1,
                lbl, color=col, fontsize=8, va='top')

    # shade periods
    ax.axvspan(0,            20*60,  alpha=0.06, color='blue',   label='×10')
    ax.axvspan(20*60,        60*60,  alpha=0.06, color='orange', label='×50')
    ax.axvspan(60*60,        80*60,  alpha=0.06, color='red',    label='×100')
    ax.axvspan(80*60,        T_SIM,  alpha=0.06, color='green',  label='×10')

    ax.set_xlabel('Time (s)')
    ax.set_ylabel(f'{title} ({ylabel})')
    ax.set_title(f'Gothenburg CTM v4 — {title}\n'
                 f'mode=links | dynamic rush hour | attr_scale=1e-5')
    ax.grid(True, alpha=0.3)
    ax.legend(title='prod_scale', fontsize=8, loc='upper left')
    fig.tight_layout()
...
print("Rendering gothenburg_links_v4.gif …")
frames = []
for t, m in snapshots:
    ...
    ax.set_title(f'mode=links  |  prod×{mult}  |  t = {t:.0f}s  ({t/60:.0f} min)')
    ax.set_xlabel('X (m)'); ax.set_ylabel('Y (m)')
    fig.tight_layout()
    frames.append(_frame_to_pil(fig))

gif_path = os.path.join(OUT_DIR, 'gothenburg_links_v4.gif')
frames[0].save(gif_path, save_all=True, append_images=frames[1:],
               duration=250, loop=0)
print("Saved → output/gothenburg_links_v4.gif")
...
print(f"\n{'='*68}")
print(f"  FINAL STATS at t = {T_SIM/60:.0f} min  [dynamic rush hour]")
print(f"{'='*68}")
