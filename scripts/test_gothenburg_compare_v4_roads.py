"""
Gothenburg CTM — mode='roads', hora de ponta dinâmica.
v4: prod_scale varia no tempo, attr_scale=1e-5 constante.
    Todos os junctions interiores injetam e absorvem (threshold=1.0).

Perfil de produção:
  0  – 20 min  →  10×  (prod_scale = 1e-4)
  20 – 60 min  →  50×  (prod_scale = 5e-4)
  60 – 80 min  → 100×  (prod_scale = 1e-3)
  80 – 120 min →  10×  (prod_scale = 1e-4)

Outputs (./output/):
  gothenburg_avg_speed_v4_roads.png
  gothenburg_queue_length_v4_roads.png
  gothenburg_load_factor_v4_roads.png
  gothenburg_total_delay_v4_roads.png
  gothenburg_roads_v4.gif
"""
import sys
import os
import io
...
# ── 7. Metric plots ───────────────────────────────────────────────────────────
METRICS = [
    ('Average Speed',       speeds,  'm/s',   'gothenburg_avg_speed_v4_roads.png'),
    ('Queue Length',        queues,  'veh',   'gothenburg_queue_length_v4_roads.png'),
    ('Load Factor (ρ/K)',   loads,   '—',     'gothenburg_load_factor_v4_roads.png'),
    ('Total Network Delay', delays,  'veh·s', 'gothenburg_total_delay_v4_roads.png'),
]

for title, vals, ylabel, fname in METRICS:
    fig, ax = plt.subplots(figsize=(11, 5))
    ax.plot(times, vals, linewidth=2, color='darkorange', zorder=3)
    ...
    ax.set_title(f'Gothenburg CTM v4 — {title}\n'
                 f'mode=roads | dynamic rush hour | attr_scale=1e-5')
...
print("Rendering gothenburg_roads_v4.gif …")
frames = []
for t, m in snapshots:
    ...
    ax.set_title(f'mode=roads  |  prod×{mult}  |  t = {t:.0f}s  ({t/60:.0f} min)')
    ax.set_xlabel('X (m)'); ax.set_ylabel('Y (m)')
    fig.tight_layout()
    frames.append(_frame_to_pil(fig))

gif_path = os.path.join(OUT_DIR, 'gothenburg_roads_v4.gif')
frames[0].save(gif_path, save_all=True, append_images=frames[1:], duration=250, loop=0)
print("Saved → output/gothenburg_roads_v4.gif")
...
print(f"\n{'='*68}")
print(f"  FINAL STATS at t = {T_SIM/60:.0f} min  [mode=roads, dynamic rush hour]")
print(f"{'='*68}")
